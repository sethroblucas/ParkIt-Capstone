...
public class EditVehicleActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    EditText make, model, year, color;
    TextView ID;
    String personID;
    String userID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText("Edit Vehicle Information");
        ...
        // EMAIL VARS
        urlAddress = "https://cgi.sice.indiana.edu/~team56/team-56/emailB.php";
        ...
        personID = intent3.getStringExtra("user_id");
        ID.setText(personID);
    ...
    //Method for showing data in drawer header
            ...
            public void onResponse(String response) {
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(EditVehicleActivity.this, error.getMessage().toString(), Toast.LENGTH_LONG).show();
                    }
                });
    ...
    //Method for getting data from vehicle data from database
    private void getData() {

        String personID = ID.getText().toString();

        String url = Config5.DATA_URL_4 + personID;
        ...
        StringRequest stringRequest = new StringRequest(url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(EditVehicleActivity.this, error.getMessage(), Toast.LENGTH_LONG).show();
                    }
                });
    ...
    //Method for getting vehicle data from database
    private void showJSON(String response) {
        final ArrayList<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
        try {
            JSONObject jsonObject = new JSONObject(response);
            JSONArray result = jsonObject.getJSONArray(Config5.JSON_ARRAY4);

            for (int i = 0; i < result.length(); i++) {
                JSONObject jo = result.getJSONObject(i);
                String str_make = jo.getString(Config5.KEY_MAKE);
                String str_model = jo.getString(Config5.KEY_MODEL);
                String str_year = jo.getString(Config5.KEY_YEAR);
                String str_color = jo.getString(Config5.KEY_COLOR);

                make.setText(str_make);
                model.setText(str_model);
    ...
    //Method for saving vehicle information
    public void SaveInformation() {
        ...
        String str_username = userName_drawer.getText().toString();

        String type = "vehicle";

        EditVehicleWorker editVehicleWorker = new EditVehicleWorker(this);
        editVehicleWorker.execute(type, str_make, str_model, str_year, str_color, personID);

        // EMAIL NOTIFICATION
        ID.setText(personID);
        name.setText(str_username);
        email.setText(str_email);
        Sender s = new Sender(EditVehicleActivity.this,urlAddress,ID,name,email);
        s.execute();



        Toast.makeText(getApplicationContext(),"Vehicle Information Saved!",Toast.LENGTH_SHORT).show();
    ...
    //Method for items in navigation drawer
    @Override
